resource "vyos_config_block_tree" "dhcp_server_lan" {
  ...
  configs = merge(
    {
      "shared-network-name lan subnet ${var.config.lan.cidr} default-router" = var.config.lan.dhcp.default_router,
      ...
      "shared-network-name lan subnet ${var.config.lan.cidr} name-server"= var.config.lan.dhcp.default_router
    ...
    {
      "failover source-address" = var.config.lan.default_router
      "failover name" = "lan"
